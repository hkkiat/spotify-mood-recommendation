...
const MostImpact: React.FC<MostImpactProps> = ({ onMostImpactChange }) => {
    ...
    return (
        <div className={styles.checkboxContainer}>
            <div className={`form-label mt-2 ${styles.moodlogquestionheader}`}>What has the most impact on you today?</div>
            <div className={styles.checkboxGroup}>
                ...
                <Checkbox id="option6Checkbox" label="Others" value="Others" checked={mostImpact === 'Others'} onChange={(checked, value) => handleMostImpactChange(checked ? value : '')} onMostImpactChange={handleMostImpactChange} />
            </div>
        </div>
    );
};
